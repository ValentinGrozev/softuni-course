...
def find_char_location(game_field, size):
    return find_locations(game_field, size, "s")[0]


def find_bomb_location(game_field, size):
    found = find_locations(game_field, size, "e")
    if len(found) > 0:
        return found[0]


def find_all_coals(game_field, size):
    coals = len(find_locations(game_field, size, "c"))
    return coals


def find_locations(game_field, size, symbol):
    occurrences = []
    for row_index in range(size):
        for col_index in range(size):
            if game_field[row_index][col_index] == symbol:
...
def main():
    ...
    char_coordinates = find_char_location(field, field_size)
    field[char_coordinates[0]][char_coordinates[1]] = "*"
