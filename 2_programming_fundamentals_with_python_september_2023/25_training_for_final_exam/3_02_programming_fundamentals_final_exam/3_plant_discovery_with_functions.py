def rate_plant(plant_information, plant_name, plant_rating):
...
def update_plant(plant_information, plant_name, plant_rarity):
    if plant_name in plant_information:
...
def reset_rating(plant_information, plant_name):
    if plant_name in plant_information:
...
def main():
    ...
    while command[0] != "Exhibition":
        if command[0] == "Rate":
            ...
            rate_plant(plant_information, plant_name, plant_rating)
        elif command[0] == "Update":
            current_plant_information = command[1].split(" - ")
            plant_name = current_plant_information[0].strip()
            plant_rarity = int(current_plant_information[1])
            update_plant(plant_information, plant_name, plant_rarity)
        elif command[0] == "Reset":
            plant_name = command[1].strip()
            reset_rating(plant_information, plant_name)
