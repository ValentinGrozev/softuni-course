...
class SummitQuestManagerApp:
    ...
    def register_climber(self, climber_type: str, climber_name: str):
        try:
            climber = self.VALID_CLIMBERS_TYPES[climber_type](climber_name)
        except KeyError:
    ...
    def check_gear(self, climber_name: str, peak_name: str, gear: List[str]):
        ...
        if gear == peak.get_recommended_gear():
            ...
            return (f"{climber_name} is not prepared to climb {peak_name}. Missing gear: "
                    f"{', '.join(sorted(g for g in peak.get_recommended_gear() if g not in gear))}.")
    ...
    def get_statistics(self):
        climbers_with_conquer_peaks = [c for c in self.climbers if len(c.conquered_peaks) > 0]
        sorted_climbers_with_conquer_peaks = sorted(climbers_with_conquer_peaks,
                                                    key=lambda c: (-len(c.conquered_peaks), c.name))
        total_peaks = len({p for c in self.climbers for p in c.conquered_peaks})

        result = f"Total climbed peaks: {total_peaks}\n"
        result += "**Climber's statistics:**\n"
        result += "\n".join(str(c) for c in sorted_climbers_with_conquer_peaks)
